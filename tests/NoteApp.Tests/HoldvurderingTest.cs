...
public class HoldvurderingTest
{
    ...
    {
        Assert.Equal(Holdsvar.Tryk, Holdvurdering.Naeste(TimeSpan.Zero, nede: false, holderAllerede: false));
    }

    [Fact]
    public void Sluppet_lige_inden_graensen_er_stadig_et_tryk()
    {
        Assert.Equal(Holdsvar.Tryk, Holdvurdering.Naeste(Kort, nede: false, holderAllerede: false));
    }
    ...
    public void Nede_men_ikke_laenge_nok_giver_ingenting()
    {
        ...
        Assert.Equal(Holdsvar.Vent, Holdvurdering.Naeste(Kort, nede: true, holderAllerede: false));
    }
    ...
    public void Graensen_taeller_med()
    {
        ...
        Assert.Equal(Holdsvar.Begynd, Holdvurdering.Naeste(Lige, nede: true, holderAllerede: false));
    }

    [Fact]
    public void Over_graensen_begynder_holdet()
    {
        Assert.Equal(Holdsvar.Begynd, Holdvurdering.Naeste(Lang, nede: true, holderAllerede: false));
    }
    ...
    public void Holdet_begynder_kun_een_gang()
    {
        ...
        Assert.Equal(Holdsvar.Vent, Holdvurdering.Naeste(Lang, nede: true, holderAllerede: true));
        Assert.Equal(Holdsvar.Vent,
            Holdvurdering.Naeste(TimeSpan.FromSeconds(30), nede: true, holderAllerede: true));
    }

    [Fact]
    public void Sluppet_efter_et_hold_afslutter_det()
    {
        Assert.Equal(Holdsvar.Slut, Holdvurdering.Naeste(Lang, nede: false, holderAllerede: true));
    }
    ...
    public void Et_hold_der_er_afsluttet_giver_ikke_et_tryk_ogsaa()
    {
        ...
        var svar = Holdvurdering.Naeste(Lang, nede: false, holderAllerede: true);
    ...
    public void En_fastsiddende_tast_slipper_af_sig_selv()
    {
        ...
        var forbi = Holdvurdering.Loft + TimeSpan.FromSeconds(1);

        Assert.Equal(Holdsvar.Slut, Holdvurdering.Naeste(forbi, nede: true, holderAllerede: true));
    }

    [Fact]
    public void Loftet_naaet_uden_at_holdet_naaede_at_begynde_er_et_tryk()
    {
        // Kan ikke ske i praksis — graensen er 350 ms og loftet tre minutter —
        // men et svar, der falder mellem to stole, maa aldrig vaere «ingenting».
        var forbi = Holdvurdering.Loft + TimeSpan.FromSeconds(1);

        Assert.Equal(Holdsvar.Tryk, Holdvurdering.Naeste(forbi, nede: true, holderAllerede: false));
    }

    [Fact]
    public void Graensen_er_kortere_end_loftet()
    {
        // Byttede de to plads, ville hvert tryk blive til en tre minutter lang
        // diktering.
        Assert.True(Holdvurdering.Graense < Holdvurdering.Loft);
